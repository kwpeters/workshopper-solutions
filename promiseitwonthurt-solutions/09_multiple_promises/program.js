...
function all(promise1, promise2) {
    var dfd = q.defer(),
        numPromisesFulfilled = 0,
        resolvedValues = [];

    promise1
        .then(
            function (value) {onPromiseFulfilled(0, value);},
            function (err) {dfd.reject();})
        .done();

    promise2
        .then(
            function (value) {onPromiseFulfilled(1, value);},
            function (err) {dfd.reject();})
        .done();

    return dfd.promise;

    function onPromiseFulfilled(index, value) {
        resolvedValues[index] = value;
        numPromisesFulfilled += 1;

        if (numPromisesFulfilled === 2) {
            dfd.resolve(resolvedValues);
        }
    }
}
